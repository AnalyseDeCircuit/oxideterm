...
impl RemoteDesktopSessionEntity {
    ...
    pub(super) fn apply_spice_event(&mut self, event: SpiceHelperEvent, _cx: &mut Context<Self>) {
        match event {
            ...
            SpiceHelperEvent::MouseMode { mode } => {
                if self.spice.mouse_mode != Some(mode) {
                    self.release_inputs();
                }
                self.spice.mouse_mode = Some(mode);
            }
            ...
            SpiceHelperEvent::FileTransferState {
                ...
                while self.spice.file_transfers.len() > 128 {
                    let oldest = self
                        .spice
                        .file_transfers
                        .iter()
                    ...
                    self.spice.file_transfers.remove(&id);
                }
            }
