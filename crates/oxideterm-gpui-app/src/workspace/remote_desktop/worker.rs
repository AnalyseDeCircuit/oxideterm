...
fn run_spice_remote_desktop_worker(
    ...
    let audio_capture = profile.session_options.audio.capture && !read_only;
    ...
    let mut adapter = SpiceRemoteDesktopAdapter::new(initial_size, monitor_layout, read_only);
    ...
    while !terminated {
        ...
        while let Ok(request) = spice_tool_request_rx.try_recv() {
            ...
            let Some(request) = adapter.map_tool_request(request) else {
                continue;
            };
...
fn deliver_spice_worker_event(
    ...
    let delivery = match delivery {
        SpiceWorkerDelivery::DisplayLayout { worker_id, layout } => {
            debug_assert_eq!(worker_id.request_id, generation);
            adapter.set_display_layout(layout);
            return false;
        }
