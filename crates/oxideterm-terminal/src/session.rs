...
fn command_output_text_from_term<T: EventListener>(
    ...
    command_output_text_from_term_with_limits(
        term,
        mark,
...
fn command_output_text_from_term_with_limits<T: EventListener>(
    term: &Term<T>,
    mark: &TerminalCommandMark,
    max_lines: usize,
    max_bytes: usize,
    ...
    let end = mark.end_line.unwrap_or_else(|| {
        scrollback.saturating_add(term.renderable_content().cursor.point.line.0.max(0) as usize)
    });
    if mark.stale || start > end || start >= term.total_lines() || max_lines == 0 || max_bytes == 0
    {
        return String::new();
    }
    let mut text = String::new();
    ...
    for absolute_line in start..=end.min(term.total_lines().saturating_sub(1)) {
        if !previous_wrapped {
            if logical_lines >= max_lines || text.len() >= max_bytes {
                break;
            }
            if logical_lines > 0 {
                text.push('\n');
        ...
        let row = &term.grid()[Line(absolute_line as i32 - scrollback as i32)];
        let wrapped = row[..]
            .last()
        ...
        let mut length = line.len().min(remaining);
        while !line.is_char_boundary(length) {
            length -= 1;
        }
        text.push_str(&line[..length]);
        if length < line.len() {
            break;
        }
