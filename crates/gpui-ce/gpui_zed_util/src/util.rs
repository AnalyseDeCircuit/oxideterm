// OxideTerm modification: use product-scoped root-execution controls and diagnostics.
...
/// `OXIDETERM_ALLOW_ROOT` environment variable.
#[cfg(unix)]
pub fn prevent_root_execution() {
    let is_root = nix::unistd::geteuid().is_root();
    let allow_root = std::env::var("OXIDETERM_ALLOW_ROOT").is_ok_and(|val| val == "true");

    if is_root && !allow_root {
        eprintln!(
            "\
Error: Running OxideTerm as root or via sudo is unsupported.
       Doing so (even once) may subtly break things for all subsequent non-root usage of OxideTerm.
       It is untested and not recommended, don't complain when things break.
       If you wish to proceed anyways, set `OXIDETERM_ALLOW_ROOT=true` in your environment."
        );
        std::process::exit(1);
    }
}
