...
impl Background {
    /// Returns the stable scalar fields consumed by renderer shader-transfer structures.
    ///
    /// This is hidden from generated documentation because it exposes the scene-to-shader wire
    /// contract rather than a user-facing color API. Explicit numeric encoding keeps renderer
    /// crates from reading private enum storage or struct padding as raw bytes.
    #[doc(hidden)]
    pub fn shader_components(&self) -> (u32, u32, Hsla, f32, [LinearColorStop; 2]) {
