...
// OxideTerm modification: nested scroll regions consume wheel events only while they can move.
...
impl Interactivity {
    ...
    fn scroll_max(&self, bounds: Bounds<Pixels>, style: &Style, window: &Window) -> Point<Pixels> {
        ...
        // Ignore tiny floating-point differences that would otherwise create a fractional
        // scroll range for content that visually fits its viewport.
            ...
            window.on_mouse_event(move |event: &ScrollWheelEvent, phase, window, cx| {
                if phase == DispatchPhase::Bubble && hitbox.should_handle_scroll(window) {
                    ...
                    if *scroll_offset != old_scroll_offset {
                        // A child overflow region that actually scrolls owns this event until it
                        // reaches a boundary, where propagation lets an ancestor take over.
                        cx.stop_propagation();
                        cx.notify(current_view);
                    }
                }
    ...
    fn nested_overflow_consumes_wheel_when_it_scrolls(cx: &mut TestAppContext) {
        ...
        impl Render for TestView {
            fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
                div()
                    .id("nested-overflow-outer")
                    .size_full()
                    .overflow_y_scroll()
                    .track_scroll(&self.outer)
                    .child(
                        div()
                            .id("nested-overflow-inner")
                            .h(px(40.))
                            .w_full()
                            .overflow_y_scroll()
                            .track_scroll(&self.inner)
                            .child(div().h(px(200.)).w_full()),
                    )
        ...
        assert_eq!(inner.offset().y, px(-30.));
        ...
        assert_eq!(inner.offset().y, px(-160.));
        assert_eq!(outer.offset().y, px(0.));
