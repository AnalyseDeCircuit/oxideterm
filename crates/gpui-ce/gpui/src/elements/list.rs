...
// OxideTerm modification: nested lists consume wheel events only while their offset changes.
...
impl StateInner {
    ...
    fn scroll(
        ...
        // Following lists can initially contain only the measured viewport. Treat the user's
        // first scroll toward the start as handled even when its pixel offset cannot move until
        // the next layout measures more content.
        let was_following = self.follow_state.is_following();
        if delta.y > px(0.) {
            self.follow_state.stop_following();
        }
        let stopped_following = was_following && !self.follow_state.is_following();
        if new_scroll_top == old_scroll_top && !stopped_following {
            return false;
        }
        ...
        window.on_mouse_event(move |event: &ScrollWheelEvent, phase, window, cx| {
            if phase == DispatchPhase::Bubble && hitbox_id.should_handle_scroll(window) {
                ...
                if list_state.0.borrow_mut().scroll(
                    ...
                    // A nested list owns this event while it can move. At its boundary, the
                    // event continues bubbling so an ancestor scroll region can take over.
                    cx.stop_propagation();
                }
            }
        });
...
mod test {
    ...
    fn nested_list_consumes_wheel_when_it_scrolls(cx: &mut TestAppContext) {
        use crate::{
            InteractiveElement, ListAlignment, ParentElement, ScrollHandle,
