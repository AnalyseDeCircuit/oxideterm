...
impl SpiceRemoteDesktopAdapter {
    pub fn new(
        ...
        Self {
            ...
            monitor_layout,
            layout: crate::SpiceDisplayLayout::default(),
    ...
    pub fn map_request(&mut self, request: RemoteDesktopHelperRequest) -> Vec<HelperRequest> {
        ...
        match request {
            ...
            RemoteDesktopHelperRequest::MouseMove { x, y } => {
                if self.mouse_mode == Some(HelperMouseMode::Client) {
                    let Some((x, y, display_id)) = self.layout.pointer_target(x, y) else {
                        return Vec::new();
                    };
            ...
            RemoteDesktopHelperRequest::PasteText { text } => {
                self.paste_text(text.expose_secret().to_owned())
            }
            RemoteDesktopHelperRequest::Text { text } => self.paste_text(text),
            ...
            // The reader publishes complete desktop snapshots, including after a hidden frame.
            RemoteDesktopHelperRequest::RequestFrame => Vec::new(),
    ...
    pub fn map_event(&mut self, event: HelperEvent) -> SpiceRemoteDesktopEventActions {
        match event {
            ...
            HelperEvent::ClipboardOffer {
                ...
                let response = (selection == HelperClipboardSelection::Clipboard)
                    .then(|| preferred_clipboard_format(&formats))
                    .flatten()
            ...
            HelperEvent::ClipboardData {
                ...
                let shared_event = (selection == HelperClipboardSelection::Clipboard)
                    .then(|| remote_clipboard_event(format, &data))
                    .flatten();
    ...
    pub fn set_display_layout(&mut self, layout: crate::SpiceDisplayLayout) {
        self.layout = layout;
    }

    pub fn map_tool_request(&self, mut request: HelperRequest) -> Option<HelperRequest> {
        if self.read_only {
            return None;
        }
        if let HelperRequest::PointerMotion { buttons, .. } = &mut request {
            if self.mouse_mode != Some(HelperMouseMode::Server) {
                return None;
            }
            *buttons = self.pressed_buttons;
        }
    ...
    fn paste_text(&mut self, text: String) -> Vec<HelperRequest> {
        let mut requests = self.map_request(RemoteDesktopHelperRequest::ClipboardText { text });
        requests.extend([
            HelperRequest::KeyCode {
                code: 0x1d,
                state: HelperKeyState::Pressed,
            },
            HelperRequest::KeyCode {
                code: 0x2f,
                state: HelperKeyState::Pressed,
...
mod tests {
    ...
    #[test]
    fn read_only_blocks_user_and_tool_writes() {
        ...
        assert!(
            adapter
                .map_request(RemoteDesktopHelperRequest::MouseButton {
        ...
        assert!(
            adapter
                .map_request(RemoteDesktopHelperRequest::PasteText {
        ...
        assert!(
            adapter
                .map_tool_request(HelperRequest::PointerMotion {
                    dx: 10,
                    dy: 20,
                    buttons: 0
                })
        ...
        assert!(matches!(
            adapter
                .map_request(RemoteDesktopHelperRequest::Close)
                .as_slice(),
            [HelperRequest::Close]
        ));
    }
    ...
    #[test]
    fn server_motion_uses_only_confirmed_relative_input_and_primary_selection_is_ignored() {
        let mut adapter = SpiceRemoteDesktopAdapter::new(
            RemoteDesktopSize {
                width: 800,
                height: 600,
        ...
        adapter.map_event(HelperEvent::MouseMode {
            mode: HelperMouseMode::Server,
        });
        assert!(
            adapter
                .map_request(RemoteDesktopHelperRequest::MouseMove { x: 200, y: 300 })
                .is_empty()
        );
        assert!(matches!(
            adapter.map_tool_request(HelperRequest::PointerMotion {
                dx: 10,
                dy: 20,
                buttons: 0
            }),
            Some(HelperRequest::PointerMotion {
                dx: 10,
                dy: 20,
                buttons: 0
            })
        ));
        let ignored = adapter.map_event(HelperEvent::ClipboardData {
            selection: HelperClipboardSelection::Primary,
            format: HelperClipboardFormat::Utf8Text,
            data: b"selection".to_vec(),
        });
        assert!(ignored.shared_event.is_none());
        let copied = adapter.map_event(HelperEvent::ClipboardData {
            selection: HelperClipboardSelection::Clipboard,
            format: HelperClipboardFormat::Utf8Text,
            data: b"copied".to_vec(),
        });
        assert!(
            matches!(copied.shared_event, Some(RemoteDesktopHelperEvent::ClipboardText { text }) if text == "copied")
        );
    }
}
