...
<p align="center">
  ...
  GPU 直接渲染。免费，无需注册。
  <br>
  <strong>零 WebView。零 OpenSSL。零遥测。零订阅。BYOK 优先。纯 Rust SSH。</strong>
</p>


<p align="center">
  <img src="https://img.shields.io/badge/version-2.0.0--gpui--preview.12-blue" alt="版本">
...
| 纯 Rust SSH 与凭证安全 | `russh` + `ring`，无 OpenSSL/libssh2；密码和 API 密钥保存在系统钥匙串，`.oxide` 使用 ChaCha20-Poly1305 + Argon2id |
...
- 每条连接有 `connecting → active → idle → link_down → 重连ing` 状态机
...
OxideSens 仍然是 BYOK 优先，native 版本把上下文构建放在进程内完成：
...
- AI 上下文来自当前工作区状态；凭据在发送给提供商前会被脱敏
- API 密钥存入系统钥匙串，不写入日志，也不会进入 IPC 帧
...
| 关注点 | 实现 |
|---|---|
| 密码与密钥 | mac系统钥匙串 / Windows Credential Manager / libsecret |
