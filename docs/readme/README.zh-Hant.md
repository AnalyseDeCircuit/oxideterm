...
<p align="center">
  <img src="https://img.shields.io/badge/version-2.0.0--gpui--preview.17-blue" alt="版本">
...
> [!WARNING]
> **OxideTerm 2.0 尚未發布穩定版。** `main` 分支目前已包含即將發布的 2.0 原始碼。現行最新穩定版仍為 `v1.6.12`；GPUI Preview 建置屬於預發布版本。
...
- 在一個桌面應用程式中管理 SSH、Telnet、序列埠、RDP/VNC、SFTP、連接埠轉發與本機 Shell
- Grace Period 重連機制可應對短暫網路中斷
- OxideSens 使用你自己的 AI 憑證，並由你核准工作區操作
- GPUI 原生介面，不使用 Electron，也不捆綁瀏覽器執行環境
