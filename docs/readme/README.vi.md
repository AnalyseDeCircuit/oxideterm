...
<p align="center">
  <strong>Ứng dụng SSH có AI cho máy chủ từ xa — ứng dụng desktop Tauri</strong>
  <br>
  Terminal SSH, SFTP, chuyển tiếp cổng, terminal nối tiếp, truyền tải trong terminal, shell cục bộ và chỉnh sửa nhẹ trong một không gian làm việc.
  <br>
  Xây dựng với Tauri & React, vận hành bởi SSH Rust thuần túy. Miễn phí. Không cần tài khoản.
  <br>
  <strong>Không Electron. Không OpenSSL. Không thu thập telemetry. Không thuê bao. Ưu tiên BYOK. SSH thuần Rust.</strong>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/version-1.6.12-blue" alt="Phiên bản">
...
<p align="center">
  🌐 <strong><a href="https://oxideterm.app">oxideterm.app</a></strong> — Tài liệu và website
</p>
...
- Quản lý terminal SSH, SFTP, chuyển tiếp cổng, truyền tải trong terminal và shell cục bộ cạnh nhau
- Tiếp tục làm việc khi mạng chập chờn nhờ cơ chế kết nối lại Grace Period
- Yêu cầu OxideSens AI kiểm tra phiên đang chạy và thực hiện thao tác đã duyệt trong không gian làm việc qua nhà cung cấp AI của bạn
...
|---|---|
| Một nút từ xa, nhiều công cụ | Terminal, SFTP, chuyển tiếp cổng, trzsz, trình soạn thảo nhẹ, giám sát và OxideSens AI cùng gắn với một không gian làm việc SSH |
| Luồng làm việc SSH ưu tiên cục bộ | SSH, SFTP, chuyển tiếp, shell cục bộ và cấu hình hoạt động không cần đăng ký; đồng bộ đám mây là tùy chọn qua [plugin chính thức](#plugin-chính-thức) |
| OxideSens AI dùng BYOK thay vì credit nền tảng | OxideSens dùng điểm truy cập OpenAI/Ollama/DeepSeek/OpenAI-compatible của bạn với MCP, RAG và thao tác không gian làm việc đã được phê duyệt |
| Kết nối lại ổn định | Grace Period kiểm tra kết nối cũ 30 giây trước khi thay thế, giúp vim/htop/yazi sống sót qua mất mạng ngắn |
| Ứng dụng native Rust thuần | Ứng dụng Tauri 2.0 native, russh 0.59 biên dịch với `ring`, không Electron, không OpenSSL/libssh2 |
| An toàn thông tin xác thực | Mật khẩu và API key ở trong keychain của hệ điều hành, metadata kết nối được niêm phong cục bộ, `.oxide` dùng ChaCha20-Poly1305 + Argon2id |

## Là gì / Không phải là gì

OxideTerm tập trung vào **không gian làm việc AI ưu tiên cục bộ cho máy chủ từ xa**. Nó dành cho người dùng muốn terminal, tệp, cổng, truyền tải, chỉnh sửa nhẹ và OxideSens AI xoay quanh máy của họ và các nút từ xa.

Nó không cố trở thành nền tảng tác nhân đám mây được lưu trữ, cũng không chỉ bán điểm benchmark kết xuất terminal. Hướng đi hẹp hơn: làm việc từ xa có cảm giác như một không gian làm việc cục bộ, không cần tài khoản OxideTerm.
...
| **AI (OxideSens)** | Trợ lý hướng mục tiêu cho kết nối đã lưu, phiên SSH đang chạy, bộ đệm terminal, đường dẫn SFTP, cài đặt và mục trong cơ sở kiến thức; có thể chẩn đoán đầu ra từ xa, chạy lệnh đã được phê duyệt, kiểm tra tệp và giải thích lỗi mà không cần tài khoản OxideTerm |
...
OxideTerm giữ bề mặt sản phẩm ưu tiên cục bộ, nhưng bên trong được xây cho luồng làm việc SSH nặng. Các chi tiết triển khai đầy đủ được giữ lại bên dưới.

<details>
<summary><strong>Kiến trúc, nội bộ SSH, reconnect, AI, forwarding, plugin và hơn nữa</strong></summary>
<br>
...
- **Không phụ thuộc OpenSSL** — toàn bộ ngăn xếp mật mã là Rust. Không còn debug "phiên bản OpenSSL nào?".
...
- **Ngữ cảnh không gian làm việc hướng mục tiêu**: xem kết nối đã lưu, phiên SSH đang chạy, bộ đệm terminal, đường dẫn SFTP, cài đặt và mục trong cơ sở kiến thức như các mục tiêu của không gian làm việc
- **Hành động đã phê duyệt**: có thể chẩn đoán đầu ra từ xa, chạy lệnh đã được phê duyệt, kiểm tra tệp và giải thích lỗi mà không cần tài khoản OxideTerm
...
- **Nhà cung cấp**: OpenAI, Ollama, DeepSeek, OneAPI, hoặc bất kỳ điểm truy cập `/v1/chat/completions` nào
...
- **API PluginContext**: 18 không gian tên — terminal, ui, lệnh, cài đặt, vòng đời, sự kiện, lưu trữ, system
...
|---|---|---|
| **Cloud Sync** | Đồng bộ tự lưu trữ được mã hóa — tải lên và nhập ảnh chụp `.oxide` qua WebDAV, HTTP JSON, Dropbox, Git hoặc S3 | [oxideterm.cloud-sync](https://github.com/AnalyseDeCircuit/oxideterm.cloud-sync) |
...
| **SSH** | russh 0.59 (`ring`) | Thuần Rust, không dùng OpenSSL/libssh2, SSH Agent |
...
- [x] Chuyển sang Rust native qua GPUI — [đang tiến hành]

---

## Hỗ trợ và bảo trì

Báo cáo lỗi và hồi quy có thể tái hiện được ưu tiên. Yêu cầu tính năng được đánh giá theo phạm vi, an toàn và mức độ phù hợp với định hướng của OxideTerm cho không gian làm việc máy chủ từ xa.
