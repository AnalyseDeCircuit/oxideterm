...
<p align="center">
  <strong>원격 서버를 위한 AI 기반 SSH 클라이언트 — Tauri 데스크톱 앱</strong>
  <br>
  SSH 터미널, SFTP, 포트 포워딩, 시리얼 터미널, 터미널 내 전송, 로컬 셸, 경량 편집을 하나의 워크스페이스에.
  <br>
...
<p align="center">
  <img src="https://img.shields.io/badge/version-1.6.12-blue" alt="버전">
...
<p align="center">
  🌐 <strong><a href="https://oxideterm.app">oxideterm.app</a></strong> — 문서와 웹사이트
</p>
...
*OxideSens가 사용자의 요청에 따라 OxideTerm 안에서 터미널을 여는 모습입니다.*
...
- SSH 터미널, SFTP, 포트 포워딩, 터미널 내 전송, 로컬 셸을 나란히 관리
...
|---|---|
| 하나의 원격 노드, 여러 도구 | 터미널, SFTP, 포트 포워딩, trzsz, 경량 편집기, 모니터링, OxideSens AI가 같은 SSH 워크스페이스에 붙어 있습니다 |
| 로컬 우선 SSH 워크플로 | SSH, SFTP, 포워딩, 로컬 셸, 설정은 가입 없이 동작하며, 클라우드 동기화는 [공식 플러그인](#공식-플러그인)으로 선택 사용합니다 |
| 플랫폼 크레딧 대신 BYOK OxideSens AI | OxideSens는 OpenAI/Ollama/DeepSeek/OpenAI 호환 엔드포인트를 사용하며 MCP, RAG, 승인된 작업 공간 동작을 지원합니다 |
...
OxideTerm은 **원격 서버를 위한 로컬 우선 AI 작업 공간**에 집중합니다. 터미널, 파일, 포트, 전송, 가벼운 편집, OxideSens AI를 자신의 머신과 원격 노드 중심에 두고 싶은 사용자를 위한 도구입니다.
...
| **AI (OxideSens)** | 저장된 연결, 라이브 SSH 세션, 터미널 버퍼, SFTP 경로, 설정, 지식 베이스 항목을 작업 공간 대상으로 보는 목표 우선 어시스턴트; OxideTerm 계정 없이 원격 출력 진단, 승인된 명령 실행, 파일 검사, 오류 설명을 수행할 수 있습니다 |
...
OxideTerm은 제품 표면은 로컬 우선으로 유지하면서도 내부는 SSH 중심 작업을 위해 설계했습니다. 구현 세부 사항은 엔지니어링 내용을 읽고 싶은 사용자를 위해 아래에 보존했습니다.

<details>
<summary><strong>아키텍처, SSH 내부, 재연결, AI, 포워딩, 플러그인 등</strong></summary>
<br>
...
- **Target-first 워크스페이스 컨텍스트**: 저장된 연결, 라이브 SSH 세션, 터미널 버퍼, SFTP 경로, 설정, 지식 베이스 항목을 워크스페이스 대상으로 봅니다
- **승인된 작업**: OxideTerm 계정 없이 원격 출력 진단, 승인된 명령 실행, 파일 검사, 오류 설명을 수행할 수 있습니다
...
OxideTerm은 운영체제가 제공하는 네이티브 WebView 런타임을 사용합니다. 대부분의 사용자는 이미 설치되어 있으며, 앱이 실행되지 않거나 에어갭 환경인 경우에만 수동 설치가 필요합니다.
...
| 계층 | 기술 | 상세 |
|---|---|---|
| **프레임워크** | Tauri 2.0 | 운영체제 WebView를 사용하는 네이티브 셸 |
| **런타임** | Tokio + DashMap 6 | 완전 비동기, 무잠금 동시 맵 |
| **SSH** | russh 0.59(`ring`) | 순수 Rust, OpenSSL/libssh2 없음, SSH Agent |
...
| 항목 | 구현 |
|---|---|
| **비밀번호** | 운영체제 키체인(macOS Keychain / Windows Credential Manager / libsecret) |
...
버그 보고와 재현 가능한 회귀 문제를 우선 처리합니다. 기능 요청은 범위, 안전성, 원격 서버 작업 공간이라는 OxideTerm의 방향성과의 일치 여부를 기준으로 검토합니다.
...
OxideTerm은 v1.0.0부터 **PolyForm Noncommercial 1.0.0**에서 **GPL-3.0**으로 전환했습니다. 이는 의도적인 선택입니다. 비상업 함정이나 경쟁 금지 조항을 단 "오픈소스 코스프레"가 아니라, 사용자, 포크, 재배포자, 상업 운영자에게 명확한 copyleft 자유를 주기 위해서입니다.

코드가 공개되어 있다고 해서 자동으로 오픈소스인 것은 아닙니다. 익숙한 오픈소스 라이선스를 내세우면서도 "재배포 금지", "재패키징 금지", "경쟁 제품 금지", "승인되지 않은 배포 플랫폼 금지" 같은 추가 조항을 붙인다면, 이는 사용자가 기대하는 오픈소스의 자유가 아니라 source-available 마케팅에 가깝습니다. OxideTerm은 비경쟁 또는 반재배포 조항을 추가하지 않습니다. GPL-3.0 조건이 전부입니다.
...
<p align="center">
  <sub>286,000줄 이상의 코드 — ⚡와 ☕로 구축</sub>
</p>
