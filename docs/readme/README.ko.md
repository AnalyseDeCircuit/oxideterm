...
<p align="center">
  <img src="https://img.shields.io/badge/version-1.5.4-blue" alt="버전">
...
|---|---|
| 하나의 원격 노드, 여러 도구 | 터미널, SFTP, 포트 포워딩, trzsz, 미니 IDE, 모니터링, OxideSens AI가 같은 SSH 워크스페이스에 붙어 있습니다 |
| 로컬 우선 SSH 워크플로 | SSH, SFTP, 포워딩, 로컬 shell, 설정은 가입 없이 동작하며, 클라우드 동기화는 [공식 플러그인](#공식-플러그인)으로 선택 사용합니다 |
| 플랫폼 크레딧 대신 BYOK OxideSens AI | OxideSens는 OpenAI/Ollama/DeepSeek/OpenAI 호환 엔드포인트를 사용하며 MCP, RAG, 승인된 workspace action을 지원합니다 |
...
OxideTerm은 **원격 서버를 위한 local-first AI workspace**에 집중합니다. 터미널, 파일, 포트, 전송, 가벼운 편집, OxideSens AI를 자신의 머신과 원격 노드 중심에 두고 싶은 사용자를 위한 도구입니다.
...
| **AI (OxideSens)** | 저장된 연결, 라이브 SSH 세션, 터미널 버퍼, SFTP 경로, 설정, 지식 베이스 항목을 워크스페이스 대상으로 보는 target-first 어시스턴트; OxideTerm 계정 없이 원격 출력 진단, 승인된 명령 실행, 파일 검사, 오류 설명을 수행할 수 있습니다 |
