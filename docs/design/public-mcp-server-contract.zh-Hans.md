...
- `tools/list` 按客户端工具组裁剪；当前实际发布基础、连接目录/详情、节点租约和命令执行/观察工具。
...
stdio bridge、终端屏幕读取、SFTP/IDE、传输、Host Tools、转发、RDP/VNC、云同步、插件、快速命令和录制仍按后续阶段实施。在对应领域的真实 broker 和生命周期接通前，这些工具不会出现在 `tools/list`，也不会用空壳结果冒充支持。
...
| 工具 | 权限 | 关键参数 | 关键结果与约束 |
|---|---|---|---|
| `terminals_open` | 终端会话 / W / 确认新会话 | `source`、`cols`、`rows`、`title?` | `source` 为 `node_ref`、一次性 `local`、`mosh`、`telnet` 或 `serial`；返回 `terminal_ref`、实际 transport、生命周期和能力。SSH 会注册独立 terminal consumer。 |
...
用户授权由客户端、工具组、资源范围、过期时间和允许的操作组合构成，不存在“给这个客户端全部权限”的通配授权。需要扩大范围时重新走 `mcp_request_access`。应用可提供一次任务、至应用重启、固定时长和持久授权；默认推荐一次任务或固定时长。

### 5.2 哪些行为必须动作确认

除工具组授权外，以下必须经 `approval_ref` 的动作确认：
