...
5. 需要时，从同一个已连接节点打开 SFTP、IDE、主机工具、图形/VNC 或转发。
...
图形/VNC 会话是附着到已连接节点的视觉应用页面。远端工作流需要桌面或图形应用，而不是终端输出时使用它。

查看器状态与终端 scrollback、保存连接数据分离。如果查看器断开，从节点上下文重连或重启图形会话；不要复制保存 SSH 配置来规避问题。
