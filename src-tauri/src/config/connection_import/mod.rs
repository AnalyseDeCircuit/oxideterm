...
//! Connection import preview and parser support.
//!
//! Importers in this module deliberately produce connection metadata only.
//! Secret-bearing fields from third-party tools are converted to warnings so
//! OxideTerm's normal first-connect Keychain flow remains the credential owner.
...
impl ImportedConnectionAuthType {
...
pub fn preview_connection_import(
    ...
    let importable = drafts.iter().filter(|draft| draft.importable).count();
    let duplicates = drafts.iter().filter(|draft| draft.duplicate).count();
    let warnings = drafts.iter().map(|draft| draft.warnings.len()).sum();
...
pub fn default_import_group(source: ConnectionImportSource) -> String {
    source.default_group().to_string()
}

pub fn unique_import_name(base_name: &str, existing_names: &HashSet<String>) -> String {
    ...
    F: FnMut(&Path, &Path) -> Result<ImportedConnectionDraft, ConnectionImportError>,
{
    ...
    for entry in WalkDir::new(root).into_iter().filter_map(Result::ok) {
        ...
        match parse_file(path, root) {
            ...
            Err(ConnectionImportError::Parse { .. }) => continue,
...
fn draft_from_fields(
    ...
    let mut tags = vec![source.tag().to_string()];
    tags.sort();
    tags.dedup();
...
mod tests {
    ...
    #[test]
    fn marks_existing_names_as_duplicates() {
        let path = fixture_path("xshell/model.xsh");
        let preview = preview_connection_import(
            ConnectionImportSource::Xshell,
            &[path.display().to_string()],
