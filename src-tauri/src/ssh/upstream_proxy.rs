...
pub fn socks5_proxy_from_env() -> Result<Option<UpstreamProxyConfig>, SshError> {
...
pub fn upstream_proxy_from_env() -> Result<Option<UpstreamProxyConfig>, SshError> {
    upstream_proxy_from_env_values(
        env::var("OXIDETERM_SOCKS5_PROXY").ok().as_deref(),
...
fn upstream_proxy_from_env_values(
    ...
    no_proxy: Option<&str>,
) -> Result<Option<UpstreamProxyConfig>, SshError> {
...
pub fn parse_socks5_proxy_value(value: &str) -> Result<UpstreamProxyConfig, SshError> {
    ...
    let (remote_dns, authority) = if let Some(rest) = trimmed.strip_prefix("socks5h://") {
        ...
        // Bare env values follow the saved proxy default: proxy-side DNS.
        (true, trimmed)
    };
...
pub fn parse_http_proxy_value(value: &str) -> Result<UpstreamProxyConfig, SshError> {
    let trimmed = value.trim();
...
async fn dial_direct_tcp(target_host: &str, target_port: u16) -> Result<TcpStream, SshError> {
...
async fn read_http_connect_response(stream: &mut TcpStream) -> Result<(), SshError> {
...
fn parse_http_connect_status(header: &str) -> Result<u16, SshError> {
...
async fn drain_socks5_bind_address(stream: &mut TcpStream, atyp: u8) -> Result<(), SshError> {
...
fn split_host_port(authority: &str) -> Result<(String, u16), SshError> {
...
fn parse_port(port: &str, invalid_port: &'static str) -> Result<u16, SshError> {
    port.parse::<u16>()
...
fn resolve_socket_addr(host: &str, port: u16) -> Result<SocketAddr, SshError> {
    let addr = format!("{host}:{port}");
    addr.to_socket_addrs()
        .map_err(|error| SshError::DnsResolution {
            address: addr.clone(),
...
fn http_io_error(error: std::io::Error) -> SshError {
    proxy_transport_error(UpstreamProxyError::HttpIo(error.to_string()))
}

fn socks_io_error(error: std::io::Error) -> SshError {
    proxy_transport_error(UpstreamProxyError::SocksIo(error.to_string()))
}

fn proxy_error<T>(error: UpstreamProxyError) -> Result<T, SshError> {
    Err(proxy_transport_error(error))
}

fn proxy_transport_error(error: UpstreamProxyError) -> SshError {
...
mod tests {
    ...
    async fn http_connect_header_timeout_uses_transport_timeout_error() {
        ...
        assert!(matches!(error, SshError::Timeout(_)));
    }
